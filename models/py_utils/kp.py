...
class kp_group(nn.Module):
    ...
    def _train(self, *xs):
        ...
        for b_ind in range(batch_size):
            ...
            cen_type = torch.ones((cen_pos.size(0),1)).float()
            cen_emb = torch.cat((cen_emb, cen_pos, cen_type), 1) 
            cen_type2 = torch.eye(cen_emb.size(0)).unsqueeze(-1)
            ...
            key_type = torch.zeros((key_pos.size(0),2)).float()
...
class GroupingLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        for b_ind in range(group_targets.size(0)):
            ...
            tmp = torch.cat((torch.zeros((cen_len,cen_len)).long(), tmp), 1)
